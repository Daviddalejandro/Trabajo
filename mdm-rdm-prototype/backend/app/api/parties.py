...
SERVICES_SQL = """
    SELECT e.enrollment_sk, bu.value_code AS business_unit, sv.value_code AS service, sv.value_name AS service_name, es.value_code AS status,
           e.enrolled_at, e.closed_at, e.source_reference, s.source_system_cd, e.captured_at, ro.value_code AS role
    ...
    LEFT JOIN mdm.party_role pr ON pr.party_role_sk=e.party_role_sk LEFT JOIN rdm.reference_value ro ON ro.value_sk=pr.role_cd
    WHERE e.party_sk=:p AND (CAST(:st AS TEXT) IS NULL OR es.value_code=:st) ORDER BY e.enrollment_sk"""


PENDING_MATCHES_SQL = """
    SELECT m.match_sk, CASE WHEN m.party_a_sk=:p THEN m.party_b_sk ELSE m.party_a_sk END AS other_party_sk,
           COALESCE(op.full_name_normalized, oo.legal_name_normalized) AS other_display_name, m.total_score, d.value_code AS decision, m.match_status,
           m.rule_version, m.matched_at,
           (SELECT json_agg(json_build_object('task_sk', t.task_sk, 'source_system_cd', s.source_system_cd, 'assignee', t.assignee, 'status', ts.value_code, 'decision', td.value_code) ORDER BY t.task_sk)
              FROM mdm.match_review_task t JOIN rdm.source_system s ON s.source_system_sk=t.source_system_cd JOIN rdm.reference_value ts ON ts.value_sk=t.task_status_cd
              LEFT JOIN rdm.reference_value td ON td.value_sk=t.decision_cd WHERE t.match_sk=m.match_sk) AS tasks
    FROM mdm.party_match m JOIN rdm.reference_value d ON d.value_sk=m.decision_cd
    JOIN mdm.party o ON o.party_sk = CASE WHEN m.party_a_sk=:p THEN m.party_b_sk ELSE m.party_a_sk END
    LEFT JOIN mdm.party_person op ON op.party_sk=o.party_sk LEFT JOIN mdm.party_org oo ON oo.party_sk=o.party_sk
    WHERE (m.party_a_sk=:p OR m.party_b_sk=:p) AND m.match_status IN ('PENDING', 'IN_REVIEW') ORDER BY m.total_score DESC, m.match_sk"""


def _summary(g: dict) -> dict:
    """Resumen ejecutivo de la Vista 360: ¿se puede contactar y por qué no?, servicios activos por UES,
    hallazgos abiertos, pares pendientes y condiciones derivadas (menor de edad, fallecido). Ley 2300/2023 arts. 3 y 5."""
    core, ct = g["core"], g["contactability"]
    elig: dict[str, dict] = {}
    for e in ct["eligibility"]:
        cur = elig.setdefault(e["purpose"], {"purpose": e["purpose"], "eligible_contacts": 0, "contacts": 0, "reasons": []})
        cur["contacts"] += 1
        if e["is_eligible"]:
            cur["eligible_contacts"] += 1
        elif e["reason"] not in cur["reasons"]:
            cur["reasons"].append(e["reason"])
    for cur in elig.values():
        cur["is_eligible"] = cur["eligible_contacts"] > 0
    services: dict[str, dict] = {}
    for sv in g["roles_relationships"]["services"]:
        cur = services.setdefault(sv["business_unit"], {"business_unit": sv["business_unit"], "active": 0, "total": 0})
        cur["total"] += 1
        if sv["status"] == "ACTIVE":
            cur["active"] += 1
    birth = core.get("birth_date")
    is_minor = None
    if birth:
        today = date.today()
        is_minor = (today.year - birth.year - ((today.month, today.day) < (birth.month, birth.day))) < 18
    return {
        "eligibility": sorted(elig.values(), key=lambda x: x["purpose"]),
        "services": sorted(services.values(), key=lambda x: x["business_unit"]),
        "open_dq_issues": sum(1 for i in g["governance"]["dq_issues"] if not i["resolved_at"]),
        "pending_matches": len(g["golden_record"]["pending_matches"]),
        "sources": len(g["sources"]),
        "is_minor": is_minor,
        "is_deceased": core.get("party_status") == "DECEASED" or bool(core.get("death_date")),
    }
...
def golden(party_sk: int, session: Session = Depends(get_session)):
    core = rows(session, """
        SELECT p.party_sk, t.value_code AS party_type, g.value_code AS golden_status, st.value_code AS party_status, p.golden_version,
               p.completeness_score, p.created_at, p.updated_at, pp.full_name_normalized, po.legal_name_normalized,
    ...
    out = {
        ...
        "identity": {
            "identifiers": L("SELECT t.value_code AS id_type, i.id_number, vs.value_code AS verification_source, i.is_verified, i.verified_at, i.is_golden, s.source_system_cd, i.captured_at FROM mdm.party_identifier i JOIN rdm.reference_value t ON t.value_sk=i.id_type_cd JOIN rdm.reference_value vs ON vs.value_sk=i.verification_source_cd JOIN rdm.source_system s ON s.source_system_sk=i.source_system_cd WHERE i.party_sk=:p"),
            "names": L("SELECT t.value_code AS name_type, n.name_value, n.valid_from, n.valid_to, s.source_system_cd FROM mdm.party_name n JOIN rdm.reference_value t ON t.value_sk=n.name_type_cd JOIN rdm.source_system s ON s.source_system_sk=n.source_system_cd WHERE n.party_sk=:p")},
        "roles_relationships": {
            ...
            "groups": L("SELECT g.group_sk, gt.value_code AS group_type, g.group_name, g.anchor_party_sk, mr.value_code AS member_role, (SELECT json_agg(json_build_object('party_sk', mm.party_sk, 'member_role', mrr.value_code, 'display_name', COALESCE(pp.full_name_normalized, po.legal_name_normalized), 'is_anchor', mm.party_sk = g.anchor_party_sk) ORDER BY mm.group_member_sk) FROM mdm.party_group_member mm JOIN rdm.reference_value mrr ON mrr.value_sk=mm.member_role_cd LEFT JOIN mdm.party_person pp ON pp.party_sk=mm.party_sk LEFT JOIN mdm.party_org po ON po.party_sk=mm.party_sk WHERE mm.group_sk=g.group_sk AND mm.party_sk<>:p) AS members FROM mdm.party_group_member m JOIN mdm.party_group g ON g.group_sk=m.group_sk JOIN rdm.reference_value gt ON gt.value_sk=g.group_type_cd JOIN rdm.reference_value mr ON mr.value_sk=m.member_role_cd WHERE m.party_sk=:p")},
        "contactability": {
            "contacts": L("SELECT l.party_contact_sk, ch.value_code AS channel, c.contact_value, c.contact_point_sk, ur.value_code AS usage_role, cf.value_code AS confirmation, og.value_code AS origin, l.is_primary, c.rne_excluded, c.is_verified, c.verified_at, l.valid_from, l.valid_to, s.source_system_cd, l.captured_at FROM mdm.party_contact_point l JOIN mdm.contact_point c ON c.contact_point_sk=l.contact_point_sk JOIN rdm.reference_value ch ON ch.value_sk=c.channel_cd JOIN rdm.reference_value ur ON ur.value_sk=l.usage_role_cd JOIN rdm.reference_value cf ON cf.value_sk=l.confirmation_status_cd JOIN rdm.reference_value og ON og.value_sk=l.origin_cd JOIN rdm.source_system s ON s.source_system_sk=l.source_system_cd WHERE l.party_sk=:p ORDER BY l.party_contact_sk"),
            "addresses": L("SELECT a.address_line, co.value_code AS country, dv.value_code AS divipola, dv.value_name AS municipality, a.is_primary, gs.value_code AS geocoding_status, a.captured_at, s.source_system_cd FROM mdm.party_address a JOIN rdm.reference_value co ON co.value_sk=a.country_cd JOIN rdm.reference_value dv ON dv.value_sk=a.divipola_cd JOIN rdm.reference_value gs ON gs.value_sk=a.geocoding_status_cd JOIN rdm.source_system s ON s.source_system_sk=a.source_system_cd WHERE a.party_sk=:p"),
        ...
        "golden_record": {
            "pending_matches": L(PENDING_MATCHES_SQL),
        ...
        "consents": {
            "consents": L("SELECT ct.value_code AS consent_type, cs.value_code AS status, c.granted_at, c.revoked_at, c.expires_at, c.evidence_ref, c.granted_by_party_sk, COALESCE(gp.full_name_normalized, go.legal_name_normalized) AS granted_by_name, s.source_system_cd FROM mdm.party_consent c LEFT JOIN mdm.party_person gp ON gp.party_sk=c.granted_by_party_sk LEFT JOIN mdm.party_org go ON go.party_sk=c.granted_by_party_sk JOIN rdm.reference_value ct ON ct.value_sk=c.consent_type_cd JOIN rdm.reference_value cs ON cs.value_sk=c.consent_status_cd JOIN rdm.source_system s ON s.source_system_sk=c.source_system_cd WHERE c.party_sk=:p AND c.valid_to IS NULL"),
