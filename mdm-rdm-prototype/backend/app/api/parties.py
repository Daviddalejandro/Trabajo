...
def golden(party_sk: int, session: Session = Depends(get_session)):
    ...
    return {
        ...
        "roles_relationships": {
            ...
            "services": L("SELECT e.enrollment_sk, bu.value_code AS business_unit, sv.value_code AS service, es.value_code AS status, e.enrolled_at, e.closed_at, e.source_reference, s.source_system_cd FROM mdm.party_service_enrollment e JOIN rdm.reference_value bu ON bu.value_sk=e.business_unit_cd JOIN rdm.reference_value sv ON sv.value_sk=e.service_cd JOIN rdm.reference_value es ON es.value_sk=e.enrollment_status_cd JOIN rdm.source_system s ON s.source_system_sk=e.source_system_cd WHERE e.party_sk=:p ORDER BY e.enrollment_sk"),
            "relationships": L("SELECT CASE WHEN r.from_party_sk=:p THEN 'OUT' ELSE 'IN' END AS direction, rt.value_code AS relationship_type, r.from_party_sk, r.to_party_sk, s.source_system_cd FROM mdm.party_relationship r JOIN rdm.reference_value rt ON rt.value_sk=r.relationship_type_cd JOIN rdm.source_system s ON s.source_system_sk=r.source_system_cd WHERE r.from_party_sk=:p OR r.to_party_sk=:p ORDER BY r.relationship_sk"),
