...
| Consola RDM | `#/rdm-consola`: siete estaciones (dominios, catálogos, campos personalizados, listas de referencia, sistemas fuente, integraciones y homologación, ciclo de vida y auditoría), recorrido guiado idempotente; navegación con contexto (dominio › catálogo › sistema en la barra «Trabajando sobre» y en la URL, «siguiente paso» contextual, desvío con retorno para registrar una fuente desde Homologación); API `POST /rdm/domains`, `POST /rdm/catalogs`, `GET /rdm/catalogs/{c}/detail`, `GET/POST /rdm/catalogs/{c}/attributes` (+`/retire`), `PUT /rdm/catalogs/{c}/values/{v}/attributes`, `POST /rdm/source-systems`, `GET/POST /rdm/integrations`, `POST /rdm/mappings/retire`, `GET /rdm/mappings/history`, `GET /rdm/audit/detail`, `GET /rdm/overview`; tabla `rdm.catalog_attribute` (migración `f7_0008`) poblada por la semilla desde el EAV | Completa | `tests/test_f7_rdm_console.py` (4), e2e «Consola RDM», DEMO.md «Consola RDM», guía §3 quater |
...
Totales verificados: backend 140 pruebas, e2e 9, `make demo` 21/21 casos + vitrina S1–S10, `npm run build` correcto.
...
| **Consola RDM** (`#/rdm-consola`, `frontend/src/pages/RdmConsole.tsx`, `backend/app/rdm/console.py`): el ciclo completo del dato de referencia desde la interfaz, con un **diccionario de campos personalizados** por catálogo (`rdm.catalog_attribute`: código, nombre, tipo TEXT/NUMBER/BOOLEAN/DATE/REGEX/CODE, obligatorio) que valida los atributos EAV al publicar o corregir un valor; alta de dominios, catálogos, sistemas fuente e integraciones; homologaciones versionadas con cierre e historial; auditoría de las cinco capas con antes y después (triggers ampliados en `f7_0008`). Los atributos de un valor sí se corrigen (describen, no identifican); código y nombre siguen inmutables (§3.7). La semilla sincroniza el diccionario desde el EAV (`sync_attribute_dictionary`) para que la suite, que migra sobre base vacía, lo tenga | Petición del autor: una consola central del RDM que muestre cómo un usuario crea dominios, catálogos, listas, campos personalizados y mapeos; DAMA-DMBOK2 Cap. 10 (Reference & Master Data), Ley 1581/2012 art. 17 (auditoría) | `console.py`, `api/rdm.py`, `seed.py`, `RdmConsole.tsx`, `App.tsx`, `tests/test_f7_rdm_console.py`, e2e |
| **Navegación con contexto en la Consola RDM** (`RdmConsole.tsx`): el objeto en curso (`dominio`, `catalogo`, `sistema`) vive en la URL (`#/rdm-consola/<estación>?catalogo=…&sistema=…&volver=…`) y en la barra «Trabajando sobre»; cambiar de estación crea entrada en el historial («atrás» funciona), cambiar el contexto con un selector la reemplaza; cada estación termina con «Siguiente: …» contextualizado; Homologación se filtra por el catálogo en contexto y ofrece «＋ registrar una fuente nueva…», que desvía a Sistemas fuente (`?volver=mapeo`, aviso de desvío) y al registrar vuelve con la fuente seleccionada y un aviso; desde un mapeo, «historial» abre el Ciclo de vida con las versiones cargadas | Petición del autor (2026-09-20): ir navegando entre las partes hasta las listas de referencia, pasar a homologar con otra fuente sin perder el catálogo y, si hace falta una fuente nueva, crearla y volver; ISO 9241-110:2020 cl. 4.4 (conformidad con las expectativas) y 4.5 (control por el usuario); DAMA-DMBOK2 Cap. 10 | `RdmConsole.tsx`, e2e «Consola RDM», DEMO.md, guía §3 quater |
| **Explicación visual de los buckets** en `#/modelo` → Cargas y buckets (`frontend/src/components/BucketsExplainer.tsx`): metáfora del archivador con cajones, cinco pasos navegables, escenarios ilustrativos (datos limpios, S1, S6, S7) y escenario «Party real» alimentado por el explorador (`GET /matching/buckets/party/{sk}`); muestra cuántos parties nunca se miran y el costo sin buckets | Petición del autor: el bloqueo es difícil de explicar en palabras; ISO 9241-110:2020 cl. 4.3 (autodescripción) | `BucketsExplainer.tsx`, `pages/Modelo.tsx`, e2e «Modelo y cargas» |
...
- Cuaderno de Colab en Drive `MDM_RDM_Prototipo/08_Colab/MDM_Prototipo_Colab.ipynb`
  (`https://colab.research.google.com/drive/1NcdyB5aqE1lvShPriwqpis5W051Uk8br`), copia del `colab/MDM_Prototipo_Colab.ipynb`
  del commit `1b30e0b` (2026-09-20, Consola RDM con recorrido guiado, vitrina S1–S10 y explicación visual de los buckets); tras cambiar el cuaderno, reemplazar la copia (subir la nueva y enviar la anterior a la papelera).
