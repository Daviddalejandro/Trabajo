    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          ["Absorbidos por merge", byStatus.MERGED ?? 0, "gray"],
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card title="Última carga por fuente (LOAD_BATCH)">
          <Table head={["Fuente", "Lote", "Modo", "Estado", "Extraídos", "Sin cambio", "Cargados", "Cuarentena", "Fin"]}
            rows={stats.last_batches.map((b: any) => [<Src v={b.source} />, b.batch_id, <Cd cat="UI" v={b.mode} />, <Badge tone={b.status === "OK" ? "green" : "red"}><Cd cat="UI" v={b.status} /></Badge>, b.extracted, b.unchanged_hash, b.loaded, b.dq_quarantined, fmtDate(b.finished_at)])} />
        </Card>
        <Card title="Matching por decisión">
          <Table head={["Decisión", "Estado", "Pares"]}
            rows={stats.matches.map((m: any) => [<Badge tone={statusTone(m.decision)}><Cd cat="CAT_MATCH_DECISION" v={m.decision} /></Badge>, <Badge tone={statusTone(m.match_status)}><Cd cat="UI" v={m.match_status} /></Badge>, m.n])} />
        ...
        <Card title="Hallazgos de calidad (PARTY_DQ_ISSUE)">
          <Table head={["Categoría", "Severidad", "Abiertos", "Total"]}
            rows={stats.dq_issues.map((d: any) => [<Cd cat="CAT_DQ_CATEGORY" v={d.category} />, <Badge tone={d.severity === "BLOCKING" ? "red" : d.severity === "WARNING" ? "yellow" : "gray"}><Cd cat="CAT_SEVERITY" v={d.severity} /></Badge>, d.open, d.total])} />
        </Card>
        <Card title="Tareas de revisión abiertas por owner">
          <Table head={["Tarea", "Par", "Fuente", "Owner", "Asignada a", "Vence"]}
            rows={tasks.map((t) => [t.task_sk, <a className="text-blue-700 underline" href={`#/stewardship/match/${t.match_sk}`}>#{t.match_sk}</a>, <Src v={t.source_system_cd} />, t.data_owner, t.assignee, <span className={t.overdue ? "text-red-700" : ""}>{fmtDate(t.due_at)}</span>])}
