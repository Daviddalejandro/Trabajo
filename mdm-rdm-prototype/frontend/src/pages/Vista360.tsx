  ...
  return (
    <div className="space-y-4">
      ...
      <LayerSection k="sources" title="1 · Sources (XREF y linaje por fila)" count={g.sources.length}>
        <Table head={["Sistema fuente", "ID externo", "Primera vez", "Última vez"]} rows={g.sources.map((s: any) => [<Src v={s.source_system_cd} />, <span className="font-mono">{s.external_id}</span>, fmtDate(s.first_seen_at), fmtDate(s.last_seen_at)])} />
        {src && (
          <div className="mt-3">
      ...
      <LayerSection k="core" title="2 · Core (fuente ganadora por campo)">
        {isPerson ? (
          <KV items={[field("Nombre", "first_name", c.first_name), field("Segundo nombre", "middle_name", c.middle_name), field("Primer apellido", "first_surname", c.first_surname), field("Segundo apellido", "second_surname", c.second_surname),
            field("Nacimiento", "birth_date", fmtDay(c.birth_date)), field("Género", "gender", <Cd cat="CAT_GENDER" v={c.gender} />), field("Estado", "party_status", <Cd cat="CAT_PARTY_STATUS" v={c.party_status} />), ["Fallecimiento", fmtDay(c.death_date)], ["Nombre normalizado (matching)", <span className="font-mono text-xs">{c.full_name_normalized ?? "—"}</span>]]} />
        ) : (
          <KV items={[field("Razón social", "legal_name", c.legal_name), field("Nombre comercial", "trade_name", c.trade_name), field("CIIU", "ciiu", <Cd cat="CAT_CIIU" v={c.ciiu} code="inline" />), field("Tipo de organización", "org_type", <Cd cat="CAT_ORG_TYPE" v={c.org_type} />), field("Estado", "party_status", <Cd cat="CAT_PARTY_STATUS" v={c.party_status} />), ["Razón social normalizada (matching)", <span className="font-mono text-xs">{c.legal_name_normalized ?? "—"}</span>]]} />
        )}
      </LayerSection>

      <LayerSection k="identity" title="3 · Identity" count={g.identity.identifiers.length}>
        <Table head={["Tipo", "Número", "Verificación", "Verificado", "Golden", "Fuente", "Capturado"]} rows={g.identity.identifiers.map((i: any) => [<Cd cat="CAT_ID_TYPE" v={i.id_type} code="inline" />, <span className="font-mono">{i.id_number}</span>, <Cd cat="CAT_VERIFICATION_SOURCE" v={i.verification_source} />, i.is_verified ? <Badge tone="green" title={fmtDate(i.verified_at)}>sí</Badge> : <Badge tone="yellow">no</Badge>, i.is_golden ? <Badge tone="pink">golden</Badge> : "", <Src v={i.source_system_cd} />, fmtDate(i.captured_at)])} />
        <h4 className="mt-3 mb-1 text-xs font-semibold uppercase text-slate-500">Nombres por tipo (legal, social, alias, anterior, comercial)</h4>
        <Table head={["Tipo", "Nombre", "Desde", "Hasta", "Fuente"]} rows={g.identity.names.map((n: any) => [<Cd cat="CAT_NAME_TYPE" v={n.name_type} />, <span className={n.valid_to ? "text-slate-400 line-through" : ""}>{n.name_value}</span>, fmtDay(n.valid_from), fmtDay(n.valid_to), <Src v={n.source_system_cd} />])} empty="Sin nombres registrados" />
      </LayerSection>
      ...
      <LayerSection k="roles" title="4 · Roles y Relaciones">
        ...
        <Table head={["Rol", "Sub-rol", "UES", "Desde", "Hasta", "Fuente"]} rows={rr.roles.map((r: any) => [<Cd cat="CAT_PARTY_ROLE" v={r.role} />, <Cd cat="CAT_PARTY_SUB_ROLE" v={r.sub_role} />, <Cd cat="CAT_BUSINESS_UNIT" v={r.business_unit} />, fmtDay(r.valid_from), fmtDay(r.valid_to), <Src v={r.source_system_cd} />])} />
        ...
        {ues.map((u) => (
          <div key={u} className="mb-2">
            ...
            <Table head={["Servicio", "Estado", "Bajo el rol", "Referencia", "Inicio", "Cierre", "Fuente"]} rows={rr.services.filter((s: any) => s.business_unit === u).map((s: any) => [<Cd cat="CAT_SERVICE" v={s.service} code="inline" />, <Badge tone={statusTone(s.status)}><Cd cat="CAT_ENROLLMENT_STATUS" v={s.status} /></Badge>, <Cd cat="CAT_PARTY_ROLE" v={s.role} />, <span className="font-mono">{s.source_reference}</span>, fmtDay(s.enrolled_at), fmtDay(s.closed_at), <Src v={s.source_system_cd} />])} />
          </div>
        ))}
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          {segTypes.map((t) => (
            <div key={t} className="rounded border p-2">
              ...
              {rr.segments.filter((s: any) => s.segment_type === t).map((s: any, i: number) => <p key={i} className={s.valid_to ? "text-slate-400 line-through" : ""}><Cd cat="CAT_SEGMENT_TYPE" v={s.segment} code="inline" /> <span className="text-xs text-slate-500"><Src v={s.source_system_cd} /> · {fmtDay(s.valid_from)}{s.valid_to ? ` → ${fmtDay(s.valid_to)}` : ""}</span></p>)}
            </div>
          ))}
        ...
        <Table head={["Dirección", "Tipo", "Con", "Tipo de party", "Desde", "Hasta", "Fuente"]} rows={rr.relationships.map((r: any) => [r.direction === "OUT" ? "→" : "←", <Cd cat="CAT_RELATIONSHIP_TYPE" v={r.relationship_type} code="inline" />, partyLink(r.other_party_sk, r.other_display_name), <Cd cat="CAT_PARTY_TYPE" v={r.other_party_type} />, fmtDay(r.valid_from), fmtDay(r.valid_to), <Src v={r.source_system_cd} />])} empty="Sin relaciones" />
        {rr.groups.map((x: any) => (
          <div key={x.group_sk} className="mt-3">
            <h4 className="mb-1 text-xs font-semibold uppercase text-slate-500">Grupo <Cd cat="CAT_GROUP_TYPE" v={x.group_type} /> · {x.group_name} <span className="normal-case">(este party: <Cd cat="CAT_GROUP_MEMBER_ROLE" v={x.member_role} />{x.anchor_party_sk === sk ? ", ancla" : ""})</span></h4>
      ...
      <LayerSection k="contact" title="5 · Contactability (finalidades por contacto y por canal)" count={ct.contacts.length}>
        ...
        <div className="flex flex-wrap gap-1">{ct.preferences.filter((p: any) => p.party_contact_sk === null).map((p: any) => <Badge key={p.pref_sk} tone={p.allowed ? "green" : "red"}><Cd cat="CAT_CONTACT_CHANNEL" v={p.channel} /> / <Cd cat="CAT_CONTACT_PURPOSE" v={p.purpose} /> {p.allowed ? "✓" : "✗"} <span className="opacity-70"><Cd cat="CAT_PREF_ORIGIN" v={p.origin} /></span></Badge>)}{!ct.preferences.some((p: any) => p.party_contact_sk === null) && <span className="text-slate-400">—</span>}</div>
        <h4 className="mt-3 mb-1 text-xs font-semibold uppercase text-slate-500">Direcciones</h4>
        <Table head={["Dirección", "Municipio (DIVIPOLA)", "País", "Principal", "Geocodificación", "Primera fuente", "Capturada"]} rows={ct.addresses.map((a: any) => [a.address_line, `${a.municipality ?? "—"} (${a.divipola})`, <Cd cat="CAT_COUNTRY" v={a.country} />, a.is_primary ? "★" : "", <Badge tone={a.geocoding_status === "PENDING" ? "yellow" : "green"}><Cd cat="CAT_GEOCODING_STATUS" v={a.geocoding_status} /></Badge>, <Src v={a.source_system_cd} />, fmtDate(a.captured_at)])} empty="Sin direcciones" />
      ...
      <LayerSection k="governance" title="6 · Governance" count={g.governance.dq_issues.length}>
        <Table head={["Categoría", "Campo", "Severidad", "Detalle", "Detectado", "Resuelto"]} rows={g.governance.dq_issues.map((i: any) => [<Cd cat="CAT_DQ_CATEGORY" v={i.category} />, i.field, <Badge tone={i.severity === "BLOCKING" ? "red" : "yellow"}><Cd cat="CAT_SEVERITY" v={i.severity} /></Badge>, <span className="text-xs">{i.detail?.message ?? JSON.stringify(i.detail)}</span>, fmtDate(i.detected_at), i.resolved_at ? fmtDate(i.resolved_at) : <Badge tone="yellow">abierto</Badge>])} empty="Sin hallazgos" />
        {g.governance.retention.length > 0 && <div className="mt-2"><p className="mb-1 text-xs uppercase text-slate-500">Retención</p><Table head={["Entidad", "Regla", "Purga después de", "Base legal", "Estado"]} rows={g.governance.retention.map((r: any) => [<><Cd cat="CAT_MDM_ENTITY" v={r.entity} /> {r.entity_sk}</>, <Cd cat="CAT_RETENTION_RULE" v={r.rule} />, fmtDay(r.purge_after), r.legal_basis, <Cd cat="UI" v={r.purge_status} />])} /></div>}
        {audit && (() => {
      ...
      <LayerSection k="golden" title="7 · Golden Record (survivorship y merges)" count={g.golden_record.survivorship.length}>
        ...
        <Table head={["Campo", "Estrategia", "Fuente ganadora", "Valor", "Decidido"]} rows={g.golden_record.survivorship.map((s: any) => [s.field_name, <Cd cat="CAT_SURVIVORSHIP_STRATEGY" v={s.strategy} />, <Src v={s.winning_source} />, <span className="font-mono text-xs">{String(s.winning_value ?? "—")}</span>, fmtDate(s.decided_at)])} empty="Sin survivorship aplicado (party candidato)" />
        {g.golden_record.merges.length > 0 && <div className="mt-3"><p className="mb-1 text-xs uppercase text-slate-500">Merges</p><Table head={["merge_sk", "Tipo", "Sobreviviente", "Absorbido", "Decidido por", "Fecha", "Estado"]} rows={g.golden_record.merges.map((m: any) => [<a className="text-blue-700 underline" href="#/stewardship/merges">{m.merge_sk}</a>, <Cd cat="CAT_MERGE_TYPE" v={m.merge_type} />, partyLink(m.surviving_party_sk), partyLink(m.merged_party_sk), m.decided_by, fmtDate(m.merged_at), m.unmerged ? <Badge tone="red">revertido</Badge> : <Badge tone="green">vigente</Badge>])} /></div>}
      </LayerSection>

      <LayerSection k="consents" title="8 · Consents (autorizaciones y solicitudes ARCO)" count={g.consents.consents.length}>
        <Table head={["Tipo", "Estado", "Otorgado", "Otorgado por", "Vence", "Revocado", "Evidencia", "Fuente"]} rows={g.consents.consents.map((x: any) => [<Cd cat="CAT_CONSENT_TYPE" v={x.consent_type} code="inline" />, <Badge tone={statusTone(x.status)}><Cd cat="CAT_CONSENT_STATUS" v={x.status} /></Badge>, fmtDate(x.granted_at), x.granted_by_party_sk ? partyLink(x.granted_by_party_sk, x.granted_by_name) : <span className="text-slate-500">titular</span>, fmtDate(x.expires_at), fmtDate(x.revoked_at), x.evidence_ref, <Src v={x.source_system_cd} />])} empty="Sin autorizaciones registradas" />
        ...
        {g.consents.arco_requests.length > 0 && <div className="mt-2"><Table head={["Solicitud", "Tipo", "Estado", "Recibida", "Vence", "Resuelta"]} rows={g.consents.arco_requests.map((r: any) => [r.request_sk, <Cd cat="CAT_ARCO_REQUEST_TYPE" v={r.arco_type} />, <Cd cat="CAT_REQUEST_STATUS" v={r.status} />, fmtDate(r.requested_at), fmtDate(r.due_at), fmtDate(r.resolved_at)])} /></div>}
      </LayerSection>
