...
function Audiences() {
  ...
  const sel = (k: keyof typeof f, opts: string[], label: string) => (
    <label className="text-sm"><span className="block text-xs text-slate-500">{label}</span>
      <select aria-label={label} value={f[k]} onChange={(e) => setF({ ...f, [k]: e.target.value })} className="w-full rounded border px-2 py-1">{opts.map((o) => <option key={o} value={o}>{o || "(cualquiera)"}</option>)}</select></label>);
  return (
    <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
      <Card title="Audiencia elegible (§10.4)">
        <div className="grid gap-2">
          {sel("purpose", ["COMMERCIAL", "BENEFITS", "COLLECTIONS"], "Finalidad")}
          {sel("channel", ["EMAIL", "PHONE", "SMS", "WHATSAPP"], "Canal")}
          {sel("role", ["", "AFFILIATE", "EMPLOYEE", "CUSTOMER", "BENEFICIARY", "VENDOR", "EMPLOYER"], "Rol")}
          ...
          {sel("service", ["", "CREDITO_SOCIAL", "SALUD_EPS", "CUOTA_MONETARIA", "TARJETA_CREDITO", "COLEGIO_MATRICULA"], "Servicio")}
          {sel("enrollment_status", ["", "ACTIVE", "SUSPENDED", "CLOSED"], "Estado del vínculo")}
      ...
      <Card title="Resultado">
        ...
        {out && (<>
          ...
          <div className="mt-2 max-h-[32rem] overflow-auto" data-testid="audience">
            <Table head={["party_sk", "Nombre", "Contacto", "Canal", "Razón"]} rows={out.items.map((x: any) => [x.party_sk, partyLink(x.party_sk, x.display_name), <span className="font-mono">{x.contact_value}</span>, x.channel, <Badge tone="green">{x.reason}</Badge>])} />
          </div>
        </>)}
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
      <Card title="Nueva solicitud ARCO">
        ...
        <div className="grid gap-2 text-sm">
          ...
          <select aria-label="Tipo ARCO" value={form.arco_type} onChange={(e) => setForm({ ...form, arco_type: e.target.value })} className="rounded border px-2 py-1">
            <option value="ACCESS">ACCESS · consulta (10 días hábiles, art. 14)</option><option value="RECTIFICATION">RECTIFICATION · reclamo (15, art. 15)</option>
            <option value="CANCELLATION">CANCELLATION · supresión (15; revoca consentimientos y marca purga)</option><option value="OPPOSITION">OPPOSITION · oposición (15)</option>
          </select>
      ...
      <Card title="Solicitudes" actions={<select aria-label="SLA" value={sla} onChange={(e) => setSla(e.target.value)} className="rounded border px-1 py-0.5 text-xs"><option value="OPEN">Abiertas</option><option value="OVERDUE">Vencidas</option><option value="RESOLVED">Resueltas</option><option value="ALL">Todas</option></select>}>
        {!items && <Spinner />}
        {items && <Table head={["#", "Party", "Tipo", "Estado", "Radicada", "Vence", "SLA", ""]} rows={items.map((r) => [r.request_sk, partyLink(r.party_sk, r.display_name), r.arco_type, <Badge tone={statusTone(r.status)}>{r.status}</Badge>, fmtDate(r.requested_at), fmtDate(r.due_at),
  ...
  return (
    <Card title="Purga simulada (§10.5): el prototipo nunca borra">
      ...
      {out && (<>
        ...
        <Table head={["Party", "Entidad", "Regla", "Purga después de", "Base legal", "Estado"]} rows={out.items.map((x: any) => [partyLink(x.party_sk, x.display_name), `${x.entity} ${x.entity_sk ?? ""}`, x.rule, fmtDay(x.purge_after), x.legal_basis, x.purge_status])} empty="Sin candidatos" />
  ...
  return (
    <Card title="Feed de cambios del golden (mismo contrato que el tópico Kafka en producción)" actions={
      ...
      {items && <div className="max-h-[36rem] overflow-auto"><Table head={["audit_sk", "Party", "Entidad", "Acción", "Versión golden", "Actor", "Fuente", "Lote", "merge_sk", "ARCO", "Fecha"]}
        rows={items.map((x) => [x.audit_sk, x.party_sk ? partyLink(x.party_sk) : "—", x.entity, <Badge tone={x.action === "MERGE" ? "pink" : x.action === "UNMERGE" ? "red" : "gray"}>{x.action}</Badge>, x.golden_version ?? "—", x.actor, x.source_system_cd ?? "—", x.batch_id ?? "—", x.merge_sk ?? "—", x.arco_request_id ?? "—", fmtDate(x.occurred_at)])} /></div>}
