...
├── frontend/e2e/          Playwright: casos B, K, L, Admin RDM, Consola RDM, Vista 360 y Modelo y cargas desde la UI
...
- **Descripciones en español.** La consola muestra el nombre de negocio de cada código canónico (roles, sub-roles, UES, tipos de relación, segmentos, tipos de nombre e identificador, canales, confirmaciones, finalidades, razones de elegibilidad, estrategias de survivorship, tipos de merge, autorizaciones, acciones de auditoría) tomado del RDM (`GET /rdm/labels`, `frontend/src/labels.tsx`); el código y el catálogo quedan en el tooltip o en línea (`code="inline"`) para trazabilidad. Los sistemas fuente conservan su código con el nombre largo en el tooltip. Cambiar una descripción es un cambio de RDM (deprecar y crear, §3.7), no de interfaz.
...
| Modelo y cargas | `#/modelo` | **Modelo relacional** leído del catálogo (37 tablas mdm + staging por capa, columnas, PK/FK, filas; clic resalta relaciones) y **Cargas y buckets**: los tres modos de entrada (FULL masiva, DELTA incremental, TX transaccional) por las mismas 7 etapas con los contadores de cada lote de `staging.LOAD_BATCH`, simulador de carga transaccional (`POST /pipeline/{fuente}/record`: un registro nativo → party creado o actualizado por XREF, buckets en los que cayó, decisión), **explicación visual de los buckets** (archivador con cajones: llega un registro → se calculan sus claves → cada clave abre un cajón → solo se compara con los vecinos → la política decide; escenarios datos limpios / S1 / S6 / S7 y el party real del explorador), estadísticas de buckets por estrategia y explorador «¿con quién se compararía este party?» |
...
| **Consola RDM** | `#/rdm-consola` | El ciclo completo del dato de referencia en siete estaciones, en el orden en que se construye el RDM (regla dura §3.1): **dominios** → **catálogos** (fuente oficial, jerarquía) → **campos personalizados** (diccionario del EAV: código, nombre, tipo de dato, obligatorio; `rdm.catalog_attribute`, migración `f7_0008`) → **listas de referencia** (valores con padre y atributos validados contra el diccionario, deprecación, corrección de atributos sin tocar código ni nombre) → **sistemas fuente** (owner y steward) → **integraciones y homologación** (campo fuente → catálogo, valor fuente → canónico versionado, cierre, prueba canónica) → **ciclo de vida y auditoría** (historial de versiones de una homologación, rehomologar, RDM_AUDIT_LOG con antes y después de las cinco capas). El **recorrido guiado** crea un catálogo sintético completo contra la API (`EXPERIENCIA` → `CAT_CANAL_PREFERIDO` → campos → valores → `APP_MOVIL` → homologaciones → deprecación y versionado → auditoría), idempotente. **Navegación con contexto**: el objeto en curso (dominio › catálogo › sistema fuente) acompaña al usuario de estación en estación, se ve en la barra «Trabajando sobre», viaja en la URL (`#/rdm-consola/<estación>?catalogo=…&sistema=…`) y contextualiza el pie «Siguiente paso»; desde las listas de referencia se pasa a la homologación del mismo catálogo y, si falta la fuente, «＋ registrar una fuente nueva…» desvía a Sistemas fuente y vuelve con la fuente seleccionada (`?volver=mapeo`). |
